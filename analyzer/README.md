...
| `app/security/engine.py` | ARP Spoofing, Flood, Port Scan, 혼잡, 링크 장애 보안 이벤트 생성 |
...
| Port Scan 의심 | `app/detection/port_scan.py`, `app/security/engine.py` | 짧은 시간 안에 같은 대상의 여러 목적지 포트로 접근하면 탐지 |
| ICMP/UDP/SYN Flood | `app/security/engine.py` | 프로토콜별 pps, bps, SYN 비율 기준으로 탐지 |
| ARP Spoofing | `app/security/engine.py` | Gateway IP의 정상 MAC과 ARP Reply의 sender MAC을 비교해 탐지 |
| ARP Reply Storm | `app/security/engine.py` | ARP Reply가 짧은 시간에 과도하게 발생하면 탐지 |
| Traffic Congestion | `app/security/engine.py` | 링크 사용률, 지연, 큐 길이, 드롭 증가 기준으로 탐지 |
| Link Failure | `app/security/engine.py` | 링크 상태가 down/failed/disabled이면 탐지 |
