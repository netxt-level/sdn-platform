...
SENSOR_MANAGER="${VM_PROJECT_DIR}/data-plane/mininet/sensor.py"
...
multipass exec "${VM_NAME}" -- \
  docker compose --profile dataplane -f "${COMPOSE_FILE}" \
  rm -f controller
multipass exec "${VM_NAME}" -- sudo python3 "${SENSOR_MANAGER}" cleanup
multipass exec "${VM_NAME}" -- sudo mn -c

echo "Controller, Sensor veth, and stale Mininet/OVS state were removed."
