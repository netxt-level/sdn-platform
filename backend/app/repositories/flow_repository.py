...
def _to_dict(flow_rule: FlowRule) -> dict[str, Any]:
    ...
    return {
        ...
        "src_ip": match.get("ipv4_src") or match.get("arp_spa"),
        "src_mac": match.get("eth_src"),
        "dst_ip": match.get("ipv4_dst") or match.get("arp_tpa"),
        "protocol": _protocol_name(match.get("ip_proto"), eth_type),
    }


def _protocol_name(ip_proto: Any, eth_type: Any = None) -> str | None:
    if eth_type in {2054, "ARP"}:
        return "ARP"
    if ip_proto == 1:
        return "ICMP"
    if ip_proto == 6:
