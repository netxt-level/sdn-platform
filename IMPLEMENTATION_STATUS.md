...
| 영역 | 경로 | 역할 |
|---|---|---|
| 분석 서버 | `analyzer/` | 패킷 캡처, 패킷 요약 생성, 이상 트래픽 탐지, 보안 이벤트 생성, 백엔드 전송 |
...
분석 서버는 패킷 캡처부터 백엔드 전송까지 기본 흐름이 구현되어 있다. 현재 구현은 지정한 네트워크 인터페이스에서 패킷을 캡처하고, 일정 시간 단위로 패킷 요약, 탐지 요약, 보안 이벤트를 만들어 백엔드 API로 전송한다.
...
| `analyzer/app/security/engine.py` | ARP Spoofing 중심 보안 이벤트와 Port Scan/ICMP Flood 보조 탐지 |
| `analyzer/app/security/runtime.py` | 보안 탐지용 rolling window와 이벤트 중복 억제 관리 |
| `analyzer/app/security/backend_contract.py` | 보안 이벤트를 백엔드/프론트엔드 payload 형태로 변환 |
...
- ARP Spoofing 보안 이벤트 탐지
- ICMP Flood, Port Scan 보조 보안 이벤트 생성
...
- 최종 보안 시나리오는 ARP Spoofing이며, Port Scan과 ICMP Flood는 보조 탐지다.
...
| `backend/app/api/security.py` | 보안 이벤트 수신/조회 API |
...
| `GET` | `/api/security/events` | Elasticsearch 보안 이벤트 조회 |
...
| `frontend/types/security.ts` | 보안 이벤트 TypeScript 타입 |
...
- 의심 호스트 목록 표시
- 의심 호스트 공격 유형별 필터링
- ARP Spoofing, ICMP Flood, Port Scan 보안 이벤트 표시
...
- 프론트는 현재 `analyzer_status`, `packet_summary`, `detection_summary`, `security_event` 메시지를 처리한다. `traffic_analysis`, `topology_update`는 과거/확장 호환용으로 남아 있다.
...
- 분석 서버 커밋에는 `analyzer/app/main.py`, `analyzer/app/detection/traffic_stats.py`, `analyzer/app/detection/port_scan.py`, `analyzer/app/security/*`, `analyzer/analyzer-backend-api.md` 포함 여부를 확인한다.
- 백엔드 커밋에는 `backend/app/api/dashboard.py`, `backend/app/api/security.py`, `backend/app/db/influxdb.py`, `backend/app/schemas/analyzer.py`, `backend/backend-api.md` 포함 여부를 확인한다.
- 프론트엔드 커밋에는 `frontend/app/page.tsx`, `frontend/app/security/events/page.tsx`, `frontend/hooks/useRealtime.ts`, `frontend/components/dashboard/ProtocolBars.tsx`, `frontend/types/analyzer.ts`, `frontend/types/security.ts` 포함 여부를 확인한다.
