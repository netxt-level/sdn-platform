...
function eventSource(event: { src_ip: string; src_mac?: string }) {
  return event.src_mac || event.src_ip || "-";
}
    ...
    () =>
      state.securityEvents.filter((event) => {
        ...
        const matchesSearch =
          ...
          eventSource(event).includes(normalizedSearch) ||
              ...
              <div className="font-mono-ui grid gap-4 text-[11px]">
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <strong className="truncate text-right">
                    <span className="text-accent">{eventSource(selectedEvent)}</span>
                ...
                {isCompletedEvent(selectedEvent) ? (
                  ...
                  <div className="grid gap-2 rounded border border-red bg-[var(--red-dim)] p-3 text-red">
                    <span>대응 정책 후보가 생성되었습니다.</span>
                    <Link
                      href="/flow-rules"
                      className="flex items-center justify-between rounded border border-red px-3 py-2"
                    >
                      <span>Flow Rule 상태 확인</span>
                      <ArrowRight size={14} />
                    </Link>
                  </div>
                ) : (
                  <div className="rounded border border-line2 bg-sidebar px-3 py-2 text-muted">
                    모니터링 이벤트입니다. 자동 대응 정책은 생성되지 않았습니다.
                  </div>
                )}
              </div>
            ) : (
                  ...
                  <div key={label} className="font-mono-ui">
                    ...
                    <div className="h-1.5 rounded bg-sidebar">
                      <div
                        ...
                        style={{ width: `${Math.min(Math.max(value * 8, 4), 100)}%` }}
                      />
                    </div>
                  </div>
                ))
